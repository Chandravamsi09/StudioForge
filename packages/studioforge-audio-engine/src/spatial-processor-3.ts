/**
 * StudioForge HRTF Spatial Convolver & Early Reflection Simulator: SpatialAudioProcessor_3
 * Implements impulse response convolution, acoustic room impulse response (RIR), and occlusion diffraction.
 */

export interface SpatialAudioProcessor_3AcousticNode {
  nodeId: string;
...
export class SpatialAudioProcessor_3Convolver {
  private acousticNode: SpatialAudioProcessor_3AcousticNode;

  constructor(node?: Partial<SpatialAudioProcessor_3AcousticNode>) {
    this.acousticNode = {
      nodeId: 'SpatialAudioProcessor_3',
      reflectionCount: 64,
