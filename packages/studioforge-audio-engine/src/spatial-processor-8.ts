/**
 * StudioForge HRTF Spatial Convolver & Early Reflection Simulator: SpatialAudioProcessor_8
 * Implements impulse response convolution, acoustic room impulse response (RIR), and occlusion diffraction.
 */

export interface SpatialAudioProcessor_8AcousticNode {
  nodeId: string;
...
export class SpatialAudioProcessor_8Convolver {
  private acousticNode: SpatialAudioProcessor_8AcousticNode;

  constructor(node?: Partial<SpatialAudioProcessor_8AcousticNode>) {
    this.acousticNode = {
      nodeId: 'SpatialAudioProcessor_8',
      reflectionCount: 64,
