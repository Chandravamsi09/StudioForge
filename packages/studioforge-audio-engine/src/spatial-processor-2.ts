/**
 * StudioForge HRTF Spatial Convolver & Early Reflection Simulator: SpatialAudioProcessor_2
 * Implements impulse response convolution, acoustic room impulse response (RIR), and occlusion diffraction.
 */

export interface SpatialAudioProcessor_2AcousticNode {
  nodeId: string;
...
export class SpatialAudioProcessor_2Convolver {
  private acousticNode: SpatialAudioProcessor_2AcousticNode;

  constructor(node?: Partial<SpatialAudioProcessor_2AcousticNode>) {
    this.acousticNode = {
      nodeId: 'SpatialAudioProcessor_2',
      reflectionCount: 64,
