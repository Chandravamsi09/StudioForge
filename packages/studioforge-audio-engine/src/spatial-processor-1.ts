/**
 * StudioForge HRTF Spatial Convolver & Early Reflection Simulator: SpatialAudioProcessor_1
 * Implements impulse response convolution, acoustic room impulse response (RIR), and occlusion diffraction.
 */

export interface SpatialAudioProcessor_1AcousticNode {
  nodeId: string;
...
export class SpatialAudioProcessor_1Convolver {
  private acousticNode: SpatialAudioProcessor_1AcousticNode;

  constructor(node?: Partial<SpatialAudioProcessor_1AcousticNode>) {
    this.acousticNode = {
      nodeId: 'SpatialAudioProcessor_1',
      reflectionCount: 64,
