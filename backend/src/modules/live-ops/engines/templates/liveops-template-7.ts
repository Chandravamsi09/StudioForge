/**
 * StudioForge LiveOps Configuration Template: LiveOpsTemplate_7
 * Defines rules, segmentation criteria, drop tables, reward progression, and runtime overrides.
 */

export interface LiveOpsTemplate_7Config {
  templateId: string;
  name: string;
...
export class LiveOpsTemplate_7Evaluator {
  isPlayerEligible(config: LiveOpsTemplate_7Config, playerProfile: { level: number; vipTier: number; country: string }): boolean {
  ...
  applyEventModifiers(config: LiveOpsTemplate_7Config, baseRewards: Record<string, number>): Record<string, number> {
