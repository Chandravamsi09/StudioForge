/**
 * StudioForge LiveOps Configuration Template: LiveOpsTemplate_14
 * Defines rules, segmentation criteria, drop tables, reward progression, and runtime overrides.
 */

export interface LiveOpsTemplate_14Config {
  templateId: string;
  name: string;
...
export class LiveOpsTemplate_14Evaluator {
  isPlayerEligible(config: LiveOpsTemplate_14Config, playerProfile: { level: number; vipTier: number; country: string }): boolean {
  ...
  applyEventModifiers(config: LiveOpsTemplate_14Config, baseRewards: Record<string, number>): Record<string, number> {
