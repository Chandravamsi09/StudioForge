/**
 * StudioForge LiveOps Configuration Template: LiveOpsTemplate_4
 * Defines rules, segmentation criteria, drop tables, reward progression, and runtime overrides.
 */

export interface LiveOpsTemplate_4Config {
  templateId: string;
  name: string;
...
export class LiveOpsTemplate_4Evaluator {
  isPlayerEligible(config: LiveOpsTemplate_4Config, playerProfile: { level: number; vipTier: number; country: string }): boolean {
  ...
  applyEventModifiers(config: LiveOpsTemplate_4Config, baseRewards: Record<string, number>): Record<string, number> {
