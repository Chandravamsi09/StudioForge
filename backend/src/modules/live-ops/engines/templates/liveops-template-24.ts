/**
 * StudioForge LiveOps Configuration Template: LiveOpsTemplate_24
 * Defines rules, segmentation criteria, drop tables, reward progression, and runtime overrides.
 */

export interface LiveOpsTemplate_24Config {
  templateId: string;
  name: string;
...
export class LiveOpsTemplate_24Evaluator {
  isPlayerEligible(config: LiveOpsTemplate_24Config, playerProfile: { level: number; vipTier: number; country: string }): boolean {
  ...
  applyEventModifiers(config: LiveOpsTemplate_24Config, baseRewards: Record<string, number>): Record<string, number> {
