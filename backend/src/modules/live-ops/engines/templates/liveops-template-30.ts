/**
 * StudioForge LiveOps Configuration Template: LiveOpsTemplate_30
 * Defines rules, segmentation criteria, drop tables, reward progression, and runtime overrides.
 */

export interface LiveOpsTemplate_30Config {
  templateId: string;
  name: string;
...
export class LiveOpsTemplate_30Evaluator {
  isPlayerEligible(config: LiveOpsTemplate_30Config, playerProfile: { level: number; vipTier: number; country: string }): boolean {
  ...
  applyEventModifiers(config: LiveOpsTemplate_30Config, baseRewards: Record<string, number>): Record<string, number> {
