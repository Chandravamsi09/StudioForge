/**
 * StudioForge LiveOps Configuration Template: LiveOpsTemplate_20
 * Defines rules, segmentation criteria, drop tables, reward progression, and runtime overrides.
 */

export interface LiveOpsTemplate_20Config {
  templateId: string;
  name: string;
...
export class LiveOpsTemplate_20Evaluator {
  isPlayerEligible(config: LiveOpsTemplate_20Config, playerProfile: { level: number; vipTier: number; country: string }): boolean {
  ...
  applyEventModifiers(config: LiveOpsTemplate_20Config, baseRewards: Record<string, number>): Record<string, number> {
