/**
 * StudioForge LiveOps Configuration Template: LiveOpsTemplate_2
 * Defines rules, segmentation criteria, drop tables, reward progression, and runtime overrides.
 */

export interface LiveOpsTemplate_2Config {
  templateId: string;
  name: string;
...
export class LiveOpsTemplate_2Evaluator {
  isPlayerEligible(config: LiveOpsTemplate_2Config, playerProfile: { level: number; vipTier: number; country: string }): boolean {
  ...
  applyEventModifiers(config: LiveOpsTemplate_2Config, baseRewards: Record<string, number>): Record<string, number> {
