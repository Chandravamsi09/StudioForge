/**
 * StudioForge LiveOps Configuration Template: LiveOpsTemplate_3
 * Defines rules, segmentation criteria, drop tables, reward progression, and runtime overrides.
 */

export interface LiveOpsTemplate_3Config {
  templateId: string;
  name: string;
...
export class LiveOpsTemplate_3Evaluator {
  isPlayerEligible(config: LiveOpsTemplate_3Config, playerProfile: { level: number; vipTier: number; country: string }): boolean {
  ...
  applyEventModifiers(config: LiveOpsTemplate_3Config, baseRewards: Record<string, number>): Record<string, number> {
