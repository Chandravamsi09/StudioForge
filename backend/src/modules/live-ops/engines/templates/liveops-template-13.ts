/**
 * StudioForge LiveOps Configuration Template: LiveOpsTemplate_13
 * Defines rules, segmentation criteria, drop tables, reward progression, and runtime overrides.
 */

export interface LiveOpsTemplate_13Config {
  templateId: string;
  name: string;
...
export class LiveOpsTemplate_13Evaluator {
  isPlayerEligible(config: LiveOpsTemplate_13Config, playerProfile: { level: number; vipTier: number; country: string }): boolean {
  ...
  applyEventModifiers(config: LiveOpsTemplate_13Config, baseRewards: Record<string, number>): Record<string, number> {
