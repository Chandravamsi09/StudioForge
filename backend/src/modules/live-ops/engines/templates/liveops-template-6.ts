/**
 * StudioForge LiveOps Configuration Template: LiveOpsTemplate_6
 * Defines rules, segmentation criteria, drop tables, reward progression, and runtime overrides.
 */

export interface LiveOpsTemplate_6Config {
  templateId: string;
  name: string;
...
export class LiveOpsTemplate_6Evaluator {
  isPlayerEligible(config: LiveOpsTemplate_6Config, playerProfile: { level: number; vipTier: number; country: string }): boolean {
  ...
  applyEventModifiers(config: LiveOpsTemplate_6Config, baseRewards: Record<string, number>): Record<string, number> {
