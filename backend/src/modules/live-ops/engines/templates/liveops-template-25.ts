/**
 * StudioForge LiveOps Configuration Template: LiveOpsTemplate_25
 * Defines rules, segmentation criteria, drop tables, reward progression, and runtime overrides.
 */

export interface LiveOpsTemplate_25Config {
  templateId: string;
  name: string;
...
export class LiveOpsTemplate_25Evaluator {
  isPlayerEligible(config: LiveOpsTemplate_25Config, playerProfile: { level: number; vipTier: number; country: string }): boolean {
  ...
  applyEventModifiers(config: LiveOpsTemplate_25Config, baseRewards: Record<string, number>): Record<string, number> {
