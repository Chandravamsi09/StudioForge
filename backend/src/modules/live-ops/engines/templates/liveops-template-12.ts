/**
 * StudioForge LiveOps Configuration Template: LiveOpsTemplate_12
 * Defines rules, segmentation criteria, drop tables, reward progression, and runtime overrides.
 */

export interface LiveOpsTemplate_12Config {
  templateId: string;
  name: string;
...
export class LiveOpsTemplate_12Evaluator {
  isPlayerEligible(config: LiveOpsTemplate_12Config, playerProfile: { level: number; vipTier: number; country: string }): boolean {
  ...
  applyEventModifiers(config: LiveOpsTemplate_12Config, baseRewards: Record<string, number>): Record<string, number> {
