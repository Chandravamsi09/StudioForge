/**
 * StudioForge LiveOps Configuration Template: LiveOpsTemplate_26
 * Defines rules, segmentation criteria, drop tables, reward progression, and runtime overrides.
 */

export interface LiveOpsTemplate_26Config {
  templateId: string;
  name: string;
...
export class LiveOpsTemplate_26Evaluator {
  isPlayerEligible(config: LiveOpsTemplate_26Config, playerProfile: { level: number; vipTier: number; country: string }): boolean {
  ...
  applyEventModifiers(config: LiveOpsTemplate_26Config, baseRewards: Record<string, number>): Record<string, number> {
