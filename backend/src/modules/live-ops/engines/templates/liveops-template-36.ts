/**
 * StudioForge LiveOps Configuration Template: LiveOpsTemplate_36
 * Defines rules, segmentation criteria, drop tables, reward progression, and runtime overrides.
 */

export interface LiveOpsTemplate_36Config {
  templateId: string;
  name: string;
...
export class LiveOpsTemplate_36Evaluator {
  isPlayerEligible(config: LiveOpsTemplate_36Config, playerProfile: { level: number; vipTier: number; country: string }): boolean {
  ...
  applyEventModifiers(config: LiveOpsTemplate_36Config, baseRewards: Record<string, number>): Record<string, number> {
